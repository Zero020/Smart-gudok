...
핵심 가치: 지출 투명성 확보, 합리적 소비 유도, 고정비 절감
...
**효과:** '구똑' 사용 시 중복 구독 정리만으로도 연간 약 11만 원 이상의 고정비를 절감할 수 있을 것으로 기대
...
- **가성비 탐지:** 사용자가 입력한 '이용률(가성비)'이 30% 이하인 서비스를 별도로 분류하여 해지를 권고합니다.
...
### 1. Cascading Renders 에러 해결
**문제:** 모달이 열릴 때 useEffect를 통해 상태를 초기화하는 과정에서 이중 렌더링(Cascading Renders) 경고 발생
